...
datasets. These were generated by the `baseline.qmd` vignette to ensure
...
parameters$log_cpue_q <- rep(as.numeric(parameters$log_cpue_q), data$n_index)
parameters$cpue_creep <- rep(as.numeric(parameters$cpue_creep), data$n_index)
parameters$log_cpue_tau <- rep(as.numeric(parameters$log_cpue_tau), data$n_index)
parameters$log_cpue_omega <- rep(as.numeric(parameters$log_cpue_omega), data$n_index)
```
...
map$cpue_creep <- factor(rep(NA, data$n_index))
map$log_cpue_tau <- factor(rep(NA, data$n_index))
map$log_cpue_omega <- factor(rep(NA, data$n_index))
```
