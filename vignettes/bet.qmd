...
Store the optimized model in an `opal_fit`. The fit retains the model inputs,
parameter map, optimizer output, bounds, diagnostics, and enough information to
rebuild the transient RTMB objective in a later R session. MCMC output and
derived products such as projections can be attached later with
`update_opal_fit()`.
...
The estimability result computed with the fit is stored in compact form, so it
travels with the other model diagnostics:
