...
The bundled `wcpo_bet_parameters$par_sel` values are already on the
opal/RTMB real-line scale used by `sel_logistic()` and
`sel_double_normal()`. For checking against SS3, @tbl-selectivity-scale converts
those values back to the SS3 reporting scale.
