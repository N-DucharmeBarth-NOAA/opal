...
Model switches used for the diagnostic fit:

```{r}
#| label: model-switches
#| echo: true
bet_lf_switch <- 1L
bet_lf_var_adjust <- 80
bet_estimate_lf_selectivity <- TRUE
bet_estimate_init_devs <- TRUE
```
...
var_adjust_scalars[] <- bet_lf_var_adjust
...
`factor(NA)` are fixed at their initial values. Quarterly recruitment deviations
are fixed here to keep this diagnostic fit focused on scale and selectivity;
freeing all quarterly deviations as fixed effects can create high-gradient early
recruitment spikes and a misleading initial biomass trajectory. Initial age
deviations are estimated because the early CPUE observations are informative
about the starting age structure.
...
if (bet_estimate_lf_selectivity) {
...
map <- list(
  ...
  # Estimate all CPUE catchabilities so each index can fit its observed scale.
  ...
  init_rdev_a = if (bet_estimate_init_devs) {
    factor(seq_len(data$n_age))
  } else {
    factor(rep(NA, data$n_age))
  }
)
```
...
#| message: false
#| eval: true
get_par_table(obj, parameters, map, lower = Lwr, upper = Upr, grad_tol = 1e-2)
```
...
if (data$cpue_switch == 0) {
  ...
  for (idx in seq_len(data$n_index)) {
    ...
    lines(log(obj$report(obj$env$last.par.best)$cpue_pred[rows]), lwd = 2)
    for (s in 1:5) lines(obj$simulate(obj$env$last.par.best)$cpue_log_obs[rows], col = "gray70")
  }
}
```
