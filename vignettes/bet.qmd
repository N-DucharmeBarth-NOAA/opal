...
par_sel_start <- as.matrix(wcpo_bet_parameters$par_sel)

# BET double-normal width starts are stored on the SS3-style width scale.
# opal's double-normal uses exp(width) * sd(length)^2, so shift width
# columns by log(sd(length)) before passing them to sel_double_normal().
par_sel_start[, 3:4] <- par_sel_start[, 3:4] - log(sd(data$len_mid))
...
The bundled `wcpo_bet_parameters$par_sel` values are converted to the
opal/RTMB real-line scale used by `sel_logistic()` and `sel_double_normal()`
before fitting. For checking against SS3, @tbl-selectivity-scale converts the
starting values back to the SS3 reporting scale.

```{r}
#| label: tbl-selectivity-scale
#| echo: true
#| message: false
#| tbl-cap: BET selectivity parameters converted back to SS3 scale.
convert_rtmb_selex_to_ss3(parameters$par_sel, data$sel_type_f, data$len_mid) %>%
...
#| fig-height: 8
#| fig-width: 10
#| fig-cap: Selectivity-at-age by fleet after integrating length selectivity over the age-length key.
rep <- obj$report()
sel_fya <- rep$sel_fya
