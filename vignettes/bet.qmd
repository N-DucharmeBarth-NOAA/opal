...
`sel_double_normal()`. For checking against SS3, the table below converts those
values back to the SS3 reporting scale.

```{r}
#| label: check-selectivity-scale
#| echo: true
#| message: false
convert_rtmb_selex_to_ss3(parameters$par_sel, data$sel_type_f, data$len_mid) %>%
  mutate(across(where(is.numeric), ~ round(.x, 3))) %>%
  knitr::kable(caption = "BET selectivity parameters converted back to SS3 scale.")
```
