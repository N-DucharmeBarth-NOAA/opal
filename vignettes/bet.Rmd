...
The `opal` package is an R package that contains the WCPO bigeye tuna (BET)
operating model coded using RTMB. This page provides examples using the `opal`
BET model.
...
# Selectivity initial values: zero matrix (all on real-line scale)
par_sel <- matrix(0, nrow = data$n_fishery, ncol = 6)
...
(`bet_model`), the AD object is created using RTMB's `MakeADFun` function:
...
```{r opt-plots, echo=TRUE, message=FALSE, fig.height=4, fig.width=7}
# CPUE fit
...
ggplot(sel_df, aes(x = real_age, y = selectivity)) +
  ...
  labs(x = "Age (years)", y = "Selectivity",
       title = "Selectivity-at-Age by Fleet") +
  ylim(0, 1)
```
