...
data <- prep_lf_data(data, lf_wide, lf_keep_fisheries = c(8:14),
...
data$lf_switch <- 1 # skip length comps (removal only)
...
Lwr[grep("par_sel", names(obj$par))] <- rep(-4, length(grep("par_sel", names(obj$par))))
Upr[grep("par_sel", names(obj$par))] <- rep(4, length(grep("par_sel", names(obj$par))))
...
if(data$wf_switch == 0){
  ...
  df_wf <- left_join(df_wf_obs, df_wf_pred, by = c("year", "id", "fishery", "weight"))
