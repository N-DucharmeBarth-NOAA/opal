...
if (data$cpue_switch == 0) {
  ...
  df <- data$cpue_data %>%
    mutate(
      pred  = rep$cpue_pred,
      sigma = rep$cpue_sigma,
      month = factor(month, levels = sort(unique(month)), labels = paste0("Month: ", sort(unique(month)))),
      fishery = factor(fishery, labels = paste0("Fishery: ", sort(unique(fishery))))
    )
  
  ggplot(df, aes(x = year)) +
    geom_linerange(aes(ymin = exp(log(value) - sigma),
                       ymax = exp(log(value) + sigma),
                       color = "Observed"), alpha = 0.6) +
    geom_point(aes(y = value, color = "Observed")) +
    geom_line(aes(y = pred, color = "Predicted"), linewidth = 0.8) +
