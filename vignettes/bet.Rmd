...
# Attach to data list
data$lf_switch         <- 1L  # multinomial offset form (recommended default)
...
# All plain R, no AD contamination
...
# Split row indices by fishery so we index lf_obs_in correctly
...
#     obs <- data$lf_obs_in[rows[i], ]   # correct row in the global matrix
#     if (bmin > 1) {
...
# Build lf_obs_list as before for reporting...
...
# Keep in nested sub-list for plain-R access in get_length_like
data$lf_obs_data <- list(obs = lf_obs_list)

# ALSO store as a flat vector — this is what OBS will mark
# Same order: fishery 1 row 1 bins, fishery 1 row 2 bins, ..., fishery 2 row 1 bins, ...
data$lf_obs_flat <- unlist(lapply(lf_obs_list, function(m) as.numeric(t(m))))

# Number of bins per observation (same for all obs when bmin=1, bmax=n_len)
...
parameters <- list(
  ...
  log_lf_tau = log(rep(1, data$n_fishery)),
...
data$lf_switch <- 1
data$lf_switch <- 2 # fails - need to normalise LFs (sum to 1) before passing to data
data$lf_switch <- 3
obj <- MakeADFun(func = cmb(opal_model, data), parameters = parameters, map = map)
obj$simulate()$lf_obs_flat
