...
# parameters$par_sel[8,] <- as.matrix(wcpo_bet_parameters$par_sel)[8,]
```
...
if (data$cpue_switch == 0) {
  ...
  ggplot(df, aes(x = year)) +
    # Simulated replicates: thin grey lines, one per sim_id
    geom_line(data = df_sim,
              aes(y = sim, group = sim_id),
              colour = "grey60", alpha = 0.25, linewidth = 0.3) +
    # Observed: points + SE bars
    ...
    labs(x = "Year", y = "CPUE", colour = NULL,
         subtitle = paste0(n_sim, " posterior-predictive simulations (grey)")) +
